"use client";

import { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/contexts/auth-context";
import AuthDialogs from "@/components/auth/auth-dialogs";
import Image from "next/image";
import dynamic from "next/dynamic";
import { ArrowLeft, Gamepad2 } from "lucide-react";

// Lazy load Game client-side only
const Game = dynamic(() => import("@/components/pacman").then(mod => ({ default: mod.PacmanGame })), {
  ssr: false,
});

export default function PacmanCompact() {
  const [isGamePlaying, setIsGamePlaying] = useState(false);
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false);
  const { user } = useAuth();

  const handlePlay = () => {
    if (user) {
      setIsGamePlaying(true);
    } else {
      setIsAuthModalOpen(true);
    }
  };

  const handleBackToPreview = () => {
    setIsGamePlaying(false);
  };

  // Listen for game events
  useEffect(() => {
    const handleGameReset = (event: CustomEvent) => {
      console.log('Game reset event received:', event.detail);
      // Reset the game to preview state
      setIsGamePlaying(false);
    };

    const handleGameQuit = (event: CustomEvent) => {
      console.log('Game quit event received:', event.detail);
      // Quit the game and return to preview state
      setIsGamePlaying(false);
    };

    window.addEventListener('gameReset', handleGameReset as EventListener);
    window.addEventListener('gameQuit', handleGameQuit as EventListener);
    
    return () => {
      window.removeEventListener('gameReset', handleGameReset as EventListener);
      window.removeEventListener('gameQuit', handleGameQuit as EventListener);
    };
  }, []);

  return (
    <>
      {isGamePlaying ? (
        // Game Playing State
        <div className="relative h-full w-full bg-black">
          {/* Back Button */}
          <div className="absolute top-4 left-4 z-50">
            <Button
              variant="ghost"
              size="sm"
              onClick={handleBackToPreview}
              className="h-8 w-8 rounded-full bg-black/50 hover:bg-black/70 text-white border border-white/20"
            >
              <ArrowLeft className="h-4 w-4" />
            </Button>
          </div>
          
          {/* Game Component */}
          <div className="w-full h-full relative">
            <Game player={user} />
          </div>
        </div>
      ) : (
        // Game Preview State
        <div className="relative h-full w-full flex flex-col justify-center p-6 overflow-hidden">
          {/* Background - Pacman Game Preview */}
          <div className="absolute inset-0 w-full h-full flex items-center justify-center">
            <Image
              src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNDhvNGd6dXc1ejRlMjFlemNwcnc4MnFzbHNieGtoNXFld3llYm5uNSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/cyMqOH8rjgDHG/giphy.gif"
              alt="Pimp Catcher background"
              width={500}
              height={400}
              className="h-full w-auto object-contain"
              unoptimized
            />
          </div>
          
          {/* Dark overlay for text readability */}
          <div className="absolute inset-0 bg-black/60" />
          
          {/* Content */}
          <div className="relative z-10 text-center">
            <h2 className="text-2xl font-bold text-white mb-2 font-oswald drop-shadow-2xl">
              🎮 Play Pimp Catcher
            </h2>
            

            {/* Game Instructions */}
            <div className="text-center mt-8 mb-8 w-1/2 mx-auto">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-2xl blur-lg"></div>
                <div className="relative bg-black/30 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                  <div className="flex items-center justify-center gap-2 mb-4">
                    <Gamepad2 className="w-5 h-5 text-primary" />
                    <div className="text-lg text-white font-oswald">How to Play:</div>
                  </div>
                  <div className="text-sm text-white/80 space-y-2 text-left">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-primary rounded-full"></div>
                      <span>Use arrow keys or joystick to move</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-secondary rounded-full"></div>
                      <span>Eat all dots to clear the level</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-red-400 rounded-full"></div>
                      <span>Avoid the ghosts!</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-yellow-400 rounded-full"></div>
                      <span>Eat power pellets to turn the tables</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <Button
              onClick={handlePlay}
              className="
                font-pricedown text-lg text-green-700 bg-white
                hover:bg-gray-100 hover:scale-105 transition-all duration-300
                px-6 py-2 rounded-lg shadow-lg
              "
            >
              {user ? "Play Now" : "Sign In to Play"}
            </Button>
          </div>
        </div>
      )}

      {/* Auth Dialog */}
      <AuthDialogs 
        isOpen={isAuthModalOpen} 
        onClose={() => setIsAuthModalOpen(false)}
        defaultTab="login"
      />
    </>
  );
}
