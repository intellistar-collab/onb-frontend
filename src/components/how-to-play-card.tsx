...
const HowToPlayCard = ({ title, subtitle, instructions }: HowToPlay) => {
  return (
    <div className="border shadow p-3 md:p-6 rounded-2xl">
      <h2 className="font-oswald text-center text-lg md:text-xl">{subtitle}</h2>
      <h1 className="font-oswald text-center text-lg md:text-xl">{title}</h1>
      <ul className="pt-2 md:pt-4 space-y-2 md:space-y-4">
        {instructions.map((instruction, index) => (
          <li key={index} className="flex flex-col gap-2 md:gap-4 items-center">
            ...
            <span className="font-oswald h-10 w-10 md:h-14 md:w-14 flex items-center justify-center text-sm md:text-base">{String.fromCharCode(65+index)}</span>
            <p className="text-xs">{instruction.text}</p>
            </div>
            {instruction.image && (
